...
## Creating circuit to implement matchgate for TFIM using PennyLane

def make_matchgate(params, wire):
    i = wire
    theta_0, theta_1, theta_2, theta_3 = params
    qml.RZ(theta_0, wires=i)
    qml.RZ(theta_0, wires=i + 1)
    ...
    qml.RX(-np.pi/2, wires=i)
    qml.RX(-np.pi/2, wires=i + 1)
    qml.RZ(theta_3, wires=i)
    qml.RZ(theta_3, wires=i + 1)
